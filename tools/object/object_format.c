...
#define OBJECT_HEADER_SIZE ((size_t)64)
#define OBJECT_SECTION_SIZE ((size_t)40)
#define OBJECT_SYMBOL_SIZE ((size_t)16)
    ...
    for (size_t i = 0; i < object->section_count; ++i) {
        ...
        if (section->name == NULL || section->name[0] == '\0' ||
            ...
            ((section->flags & CVM_OBJECT_SECTION_WRITE) != 0 &&
             (section->flags & CVM_OBJECT_SECTION_EXECUTE) != 0)) return 0;
    }
    for (size_t i = 0; i < object->symbol_count; ++i) {
        const CvmObjectSymbol *symbol = &object->symbols[i];
        int defined = (symbol->flags & CVM_OBJECT_SYMBOL_DEFINED) != 0;
        if (symbol->name == NULL || symbol->name[0] == '\0' ||
            (symbol->flags & ~(uint16_t)(CVM_OBJECT_SYMBOL_GLOBAL |
                                         CVM_OBJECT_SYMBOL_DEFINED |
                                         CVM_OBJECT_SYMBOL_ENTRY)) != 0 ||
            (defined && symbol->section_index >= object->section_count) ||
            (!defined && symbol->section_index != CVM_OBJECT_UNDEFINED_SECTION) ||
            ((symbol->flags & CVM_OBJECT_SYMBOL_ENTRY) != 0 && !defined)) return 0;
    }
    return 1;
}
                     ...
                     size_t error_size)
{
    ...
    size_t section_bytes, symbol_bytes, cursor;
    ...
    for (size_t i = 0; i < object->section_count; ++i) {
        if (!add_size(cursor, object->sections[i].source_size, &cursor)) {
            set_error(error, error_size, "object source payload overflow");
            return 0;
        }
    }
    ...
    size_t source_cursor = string_offset + string_size;
    for (size_t i = 0; i < object->section_count; ++i) {
        const CvmObjectSection *section = &object->sections[i];
        ...
        write_u64(entry + 0x10, source_cursor);
        write_u64(entry + 0x18, section->source_size);
        size_t length = strlen(section->name) + 1;
        memcpy(data + string_cursor, section->name, length);
        string_cursor += length;
        memcpy(data + source_cursor, section->source, section->source_size);
        source_cursor += section->source_size;
    }
    for (size_t i = 0; i < object->symbol_count; ++i) {
        const CvmObjectSymbol *symbol = &object->symbols[i];
                     ...
                     uint32_t relative, char **result)
{
    ...
    const void *end = memchr(data + start, 0, maximum);
    if (end == NULL) return 0;
    size_t length = (const uint8_t *)end - (data + start);
    if (start + length >= file_size || length == 0) return 0;
                    ...
                    size_t error_size)
{
    ...
    *object = (CvmObjectFile){0};
    ...
    uint64_t encoded_size = read_u64(data + 0x38);
    size_t section_bytes, symbol_bytes;
    ...
    if (object->sections == NULL || (symbol_count != 0 && object->symbols == NULL)) {
        free(data);
    ...
    for (size_t i = 0; i < section_count; ++i) {
        ...
        uint64_t source_offset = read_u64(entry + 0x10);
        uint64_t source_size = read_u64(entry + 0x18);
        if (!read_name(data, size, string_offset, string_size,
                       read_u32(entry + 0x00), &section->name) ||
            !range_valid(size, source_offset, source_size) ||
            source_size > SIZE_MAX) {
            free(data);
            cvm_object_destroy(object);
            set_error(error, error_size, "invalid object section");
            return 0;
        }
        section->source = copy_string((const char *)data + (size_t)source_offset,
                                      (size_t)source_size);
        section->source_size = (size_t)source_size;
        if (section->source == NULL) {
            free(data);
            cvm_object_destroy(object);
            set_error(error, error_size, "cannot allocate section source");
            return 0;
        }
    }
