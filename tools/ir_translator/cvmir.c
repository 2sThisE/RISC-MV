                               ...
                               const char *line_text, size_t line)
{
    ...
    if (!parser->options->allow_foreign_triple &&
        strcmp(module->target_triple, RARCH_M64_LLVM_TARGET_TRIPLE) != 0) {
        set_error(parser, line, 1,
                  "IR target triple '%s' is not '%s'",
                  module->target_triple, RARCH_M64_LLVM_TARGET_TRIPLE);
        return 0;
    }
    return 1;
}
...
{
    emit(generator, "; generated by legacy cvmir for RArchM64 %s\n",
