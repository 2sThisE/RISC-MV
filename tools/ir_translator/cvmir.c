                               ...
                               const char *line_text, size_t line)
{
    ...
    if (!parser->options->allow_foreign_triple &&
        strcmp(module->target_triple, RISC_VM_LLVM_TARGET_TRIPLE) != 0) {
        set_error(parser, line, 1,
                  "IR target triple '%s' is not '%s'",
                  module->target_triple, RISC_VM_LLVM_TARGET_TRIPLE);
        return 0;
    }
    return 1;
}
                             ...
                             const char *line_text, size_t line)
{
    ...
    if (!parser->options->allow_foreign_triple &&
        ...
        set_error(parser, line, 1,
                  "IR DataLayout does not match RISC-VM LLVM DataLayout v1");
        return 0;
    }
    return 1;
}
...
{
    emit(generator, "; generated by legacy cvmir for RISC-VM %s\n",
