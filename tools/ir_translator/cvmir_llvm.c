...
static int emit_memory_intrinsic(Generator *generator,
                                 const FunctionLayout *layout,
                                 LLVMValueRef instruction,
                                 const char *target_func)
{
    ...
    if (argument_count < 3) {
        fail(generator, "memory intrinsic requires at least 3 arguments");
        return 0;
    }
    static const char *const arg_regs[] = {"R0", "R1", "R2"};
    for (unsigned i = 0; i < 3; ++i) {
        LLVMValueRef arg = LLVMGetOperand(instruction, i);
        if (!emit_load_value(generator, layout, arg, arg_regs[i])) return 0;
        if (i == 1 && strcmp(target_func, "memset") == 0) {
            emit_zero_normalize(generator, "R1", scalar_bits(LLVMTypeOf(arg)));
        }
    }
                     ...
                     LLVMValueRef instruction)
{
    ...
    int is_indirect = (LLVMGetValueKind(called) != LLVMFunctionValueKind);
    char callee[CVMIR_NAME_MAX] = {0};
    ...
    if (!is_indirect) {
        ...
        if (strncmp(callee, "llvm.memcpy", 11) == 0)
            return emit_memory_intrinsic(generator, layout, instruction, "memcpy");
        if (strncmp(callee, "llvm.memset", 11) == 0)
            return emit_memory_intrinsic(generator, layout, instruction, "memset");
        if (strncmp(callee, "llvm.memmove", 12) == 0)
            return emit_memory_intrinsic(generator, layout, instruction, "memmove");
    }

    if (is_indirect) {
        if (!emit_load_value(generator, layout, called, "R11")) return 0;
    }
...
static int emit_module(Generator *generator, LLVMModuleRef module)
{
    ...
    for (LLVMValueRef function = LLVMGetFirstFunction(module); function != NULL;
        ...
        size_t len = 0;
        const char *raw_name = LLVMGetValueName2(function, &len);
        if (raw_name == NULL || len == 0) continue;
        char name[CVMIR_NAME_MAX];
        if (!get_name(generator, function, name)) return 0;
        if (find_cvm_intrinsic(name) != NULL) continue;
        if (strncmp(name, "llvm.memcpy", 11) == 0) {
            needs_memcpy = 1;
            continue;
        }
        if (strncmp(name, "llvm.memset", 11) == 0) {
            needs_memset = 1;
            continue;
        }
        if (strncmp(name, "llvm.memmove", 12) == 0) {
            needs_memmove = 1;
            continue;
        }
        emit(generator, ".extern %s\n.type %s, function\n", name, name);
    }
    if (needs_memcpy && LLVMGetNamedFunction(module, "memcpy") == NULL)
        emit(generator, ".extern memcpy\n.type memcpy, function\n");
    if (needs_memset && LLVMGetNamedFunction(module, "memset") == NULL)
        emit(generator, ".extern memset\n.type memset, function\n");
    if (needs_memmove && LLVMGetNamedFunction(module, "memmove") == NULL)
        emit(generator, ".extern memmove\n.type memmove, function\n");
...
static int lower_switch_instructions(LLVMModuleRef module, CvmIrError *error)
{
    LLVMPassBuilderOptionsRef options = LLVMCreatePassBuilderOptions();
    if (options == NULL) {
        set_llvm_error(error, "cannot create LLVM pass options");
        return 0;
    }
    LLVMPassBuilderOptionsSetVerifyEach(options, 1);
    LLVMErrorRef pass_error = LLVMRunPasses(
        module, "lower-switch", NULL, options);
    LLVMDisposePassBuilderOptions(options);
    if (pass_error == LLVMErrorSuccess) return 1;
    char *message = LLVMGetErrorMessage(pass_error);
    set_llvm_error(error,
                   message != NULL ? message : "cannot lower LLVM switch");
    if (message != NULL) LLVMDisposeErrorMessage(message);
    return 0;
}
                    ...
                    char **assembly, CvmIrError *error)
{
    ...
    *assembly = NULL;
    ...
    if (!lower_switch_instructions(module, error)) {
        LLVMDisposeModule(module);
        LLVMContextDispose(context);
        return 0;
    }
