...
`examples/syscall_demo.asm`은 `SYS_WRITE`를 UART 한 바이트 출력으로 처리하고 `SYS_EXIT`을 supervisor `HALT`로 처리하는 최소 게스트 커널 예제다.

```powershell
.\build\vmasm.exe .\examples\syscall_demo.asm -o .\build\examples\syscall_demo.bin
...
레이블 기반 절대/상대 분기, `R0`~`R15`와 `SP` 별칭, 숫자 표현식, 문자열 및 `.byte`/`.word`/`.dword`/`.qword`/`.ascii`/`.asciz`/`.space`/`.align`/`.org`/`.entry` 지시어를 지원한다. 전체 문법과 raw 바이너리의 진입점 제약은 `tools/assembler/README.md`를 참고한다.
