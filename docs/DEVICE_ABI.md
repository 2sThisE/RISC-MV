...
```text
.\build\main.exe -r 1024 -l .\examples\calculation.bin -d device1.dll -d device2.dll
```
...
`devices/sample_counter.c`는 VM 본체와 별도로 빌드되는 counter/IRQ 장치다.
...
`devices/display_device.c`는 guest RAM framebuffer를 DMA로 읽어 `display.present` host service에 제출하는 독립 DLL이다.
...
`devices/block_device.c`는 raw host 파일을 512바이트 LBA 장치로 노출한다. 최대 64KiB DMA READ/WRITE, 명시적 FLUSH, 비동기 worker, 완료 IRQ와 읽기 전용 모드를 지원한다.

```powershell
.\devices\build_block.ps1
.\build\vmasm.exe .\examples\block_demo.asm -o .\build\examples\block_demo.bin
...
`devices/keyboard_device.c`는 `input.keyboard` Host Service를 256-entry HID event queue, MMIO와 IRQ로 노출한다.
