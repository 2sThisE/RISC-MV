...
모든 테스트 suite는 `build/tools/test_runner.exe`에 들어간다. 각 suite는 별도 프로세스로 실행되므로 하나가 assertion으로 종료되어도 runner가 다음 suite를 계속 실행하고 마지막에 전체 결과를 반환한다.
...
```powershell
.\build\tools\test_runner.exe --list
.\build\tools\test_runner.exe --run privilege
.\build\tools\test_runner.exe --filter interrupt
.\build\tools\test_runner.exe --repeat 100
.\build\tools\test_runner.exe --filter exception --repeat 100
```
