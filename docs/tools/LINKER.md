...
- `--base`: 코드가 적재될 시작 주소. 10진수 또는 `0x` 16진수로 지정하며 4KiB 페이지 정렬이 필요하다. 생략 시 기본값은 `0x10000` (64 KiB)이다.
- `--entry`: 이미지 진입점 심볼 이름을 지정한다. 진입점 심볼은 `.text` 섹션에 존재해야 한다.
- `--map`: 심볼 및 세그먼트 배치 맵 파일 경로를 생성한다.
- `--physical-relocatable`: 물리 재배치 가능한 이미지 세그먼트로 생성한다.
