# CVM 부팅 디스크 도구 (`vmkdisk`)

`vmkdisk`는 검증된 `kernel.cvm`을 포함하는 GPT/FAT32 raw 디스크 이미지를
...
.\build\tools\vmkdisk.exe create `
    -o .\build\system.img `
    --size 64M `
    --bootloader .\build\bootloader.cvm `
    --kernel .\build\kernel.cvm
```
...
- CVM Boot Partition Type GUID
- FAT32 VBR, backup VBR, FSInfo와 두 개의 동일한 FAT
- `/BOOT/BOOT.CVM`과 `/BOOT/KERNEL.CVM`
...
누락되거나 손상된 `BOOT.CVM`, `KERNEL.CVM`을 거부한다.
