...
```powershell
.\build\tools\vmkimg.exe pack .\build\examples\kernel_stub.bin `
    -o .\build\examples\kernel_stub.exf `
    --load 0x30000 `
    --entry 0x30000 `
    --memory-size 0x1000
```
...
현재 패키저는 raw 파일 하나를 단일 LOAD 세그먼트로 만든다. EXF 규격은
최대 64개 세그먼트를 지원하며, 현재 `cvmlink`는 `.text`, `.rodata`,
`.data`, `.bss`를 별도 권한의 다중 세그먼트로 출력한다. 따라서 일반적인
C/assembly 커널은 `cvmlink`를 사용하고 `vmkimg`는 raw 이미지나 작은
부트 예제를 포장할 때 사용한다.

## 검사

```powershell
.\build\tools\vmkimg.exe inspect .\build\examples\kernel_stub.exf
```
