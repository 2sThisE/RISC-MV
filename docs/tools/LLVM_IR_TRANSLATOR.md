# Clang/LLVM RISC-VM 크로스 컴파일 경로

레거시 이름의 `cvmir`는 LLVM 22의 공식 IR parser와 verifier를 사용하고
RISC-VM ABI v1
어셈블리 또는 `CVMOBJ2` 오브젝트를 생성하는 assembly bridge다. LLVM 문법을
프로젝트가 다시 해석하지 않으며, RISC-VM instruction selection과 stack frame,
데이터 배치만 담당한다.

`cvmclang.ps1`은 Clang 호출, RISC-VM target 정규화, legacy 이름의 `cvmir`,
`vmasm`, `cvmlink`,
...
`-O0`을 사용한다. 최적화 IR은 i65 같은 RISC-VM 비정규 정수 폭을 만들 수 있으므로
...
단일 RISC-VM 명령으로 직접 내린다. 따라서 별도 런타임 구현이나 링크 심볼이
필요하지 않다.
...
현재 정식 경로는 LLVM IR에서 레거시 이름의 RISC-VM
assembler/object/linker로 이어지는 bridge다. LLVM MC/object writer, LLD와
native RISC-VM backend 포팅은 장기 검토 항목이다. 즉 현재 경로는 임시
