# Clang/LLVM CVM 크로스 컴파일 경로

`cvmir`는 LLVM 22의 공식 IR parser와 verifier를 사용하고 CVM ABI v1
어셈블리 또는 `CVMOBJ2` 오브젝트를 생성하는 assembly bridge다. LLVM 문법을
프로젝트가 다시 해석하지 않으며, CVM instruction selection과 stack frame,
데이터 배치만 담당한다.

`cvmclang.ps1`은 Clang 호출, CVM target 정규화, `cvmir`, `vmasm`, `cvmlink`,
...
```powershell
.\build\tools\cvmclang.ps1 program.c -o program.cvm
```

커널처럼 시작 코드와 entry를 직접 지정할 수도 있다.

```powershell
.\build\tools\cvmclang.ps1 kernel.c -o kernel.cvm `
...
- weak/common/external symbol과 CVM relocation
...
`-O0`을 사용한다. 최적화 IR은 i65 같은 CVM 비정규 정수 폭을 만들 수 있으므로
...
단일 CVM 명령으로 직접 내린다. 따라서 별도 런타임 구현이나 링크 심볼이
필요하지 않다.
...
P0는 LLVM IR→기존 CVM assembler/object/linker bridge를 정식 경로로 선택한다.
LLVM MC/object writer, LLD와 native CVM backend 포팅은 bridge의 ABI 회귀 결과를
기준으로 별도 진행한다. 즉 현재 경로는 임시 hand-written IR parser가 아니지만,
LLVM 자체의 native code generator도 아니다.
