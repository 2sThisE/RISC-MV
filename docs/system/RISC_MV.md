# RISC-MV 아키텍처 명칭과 호환성 정책

이 프로젝트의 공식 ISA 계열 명칭은 `RISC-MV`이며 `MV`는
`Minimal Virtualization`을 뜻한다. 현재 구현하는 64비트 아키텍처 명칭은
`RArchM64`이고 LLVM target triple은 `rarchm64-unknown-none`이다. 실행파일
포맷은 RISC-MV Executable File v1, 확장자는 `.exf`다. RISC-MV는 독자 ISA이며
RISC-V 바이너리 호환 아키텍처가 아니다.
...
|---|---|
| 확장자 | `.exf` |
| 8-byte magic | `RMVEXF01` |
| ISA architecture | `RArchM64` |
| ISA ID | little-endian `RA64` (`0x34364152`) |
| format version | 1.0 |
| ISA version | 1 |
...
유지한다. 새 public EXF 상수와 타입은 `RISC_MV_*`, `RiscMv*`를 사용하고,
64비트 ISA 고유 상수와 target은 `RARCH_M64_*`, `RArchM64`를 사용한다.
`boot_format.h`는 기존 source 이름을 새 EXF 값의 alias로 제공한다.
